...
def test(config, logger, epoch, model, test_loader, criterion):
    ...
    for batch_idx, (inputs, targets) in enumerate(test_loader):
        with torch.no_grad():
            ...
            countfire = count_fire(targets, outputs)
            ...
            if batch_idx and batch_idx % config.draw == 0:
                ...
                #plt.savefig(os.path.join(config.cache_dir, '{:03d}_{:05d}_Patch_SAN_ConvLSTM_FIRE1.png'.format(epoch, batch_idx)))
                #plt.savefig(os.path.join(config.cache_dir, '{:03d}_{:05d}_originalConfig_ConvLSTM.png'.format(epoch, batch_idx)))
                #plt.savefig(os.path.join(config.cache_dir, '{:03d}_{:05d}_non_attention_ConvLSTM_FIRE_lr04_NEW.png'.format(epoch, batch_idx)))
                #plt.savefig(os.path.join(config.cache_dir, '{:03d}_{:05d}_ConvLSTM_CSA_ALL.png'.format(epoch, batch_idx)))
                #plt.savefig(os.path.join(config.cache_dir, '{:03d}_{:05d}_ConvLSTM_SCA_ALL.png'.format(epoch, batch_idx)))
                #plt.savefig(os.path.join(config.cache_dir, '{:03d}_{:05d}_ConvLSTM_SCA_FIRE.png'.format(epoch, batch_idx)))
                #plt.savefig(os.path.join(config.cache_dir, '{:03d}_{:05d}_NA_ConvLSTM_ALL.png'.format(epoch, batch_idx)))
