...
from networks.sa.functions.utils import Dtype, Stream, load_kernel
...
def test_subtraction2_zeropad():
    ...
    y2 = unfold_i(x1).view(n, c, 1, out_height * out_width) - unfold_j(x2).view(n, c, pow(kernel_size, 2), out_height * out_width)
    ...
    assert torch.autograd.gradcheck(partial(subtraction2_zeropad, kernel_size=kernel_size, stride=stride, padding=padding, dilation=dilation), (x1, x2))
