...
from networks.sa import functional_pair as F1
...
class SAM(nn.Module):
    def __init__(self, sa_type, in_planes, rel_planes, out_planes,  share_planes=4, kernel_size=3, stride=1, dilation=1):
        ...
        if sa_type == 0:
            ...
            self.unfold_j = nn.Unfold(3, dilation=1, padding=0, stride=stride)
            self.pad = nn.ReflectionPad2d(1)
        self.aggregation = Aggregation(3, stride, (dilation * (kernel_size- 1) + 1) // 2, dilation, pad_mode=1)
