...
from networks.sa import functional_patch5 as F1
...
class SAM(nn.Module):
    def __init__(self, sa_type, in_planes, rel_planes, out_planes,  share_planes=4, kernel_size=5, stride=1, dilation=1):
        ...
        if sa_type == 0:
            ...
            self.conv_w = nn.Sequential(nn.BatchNorm2d(rel_planes * (pow(5, 2) + 1)), 
                                        nn.ReLU(inplace=True),
                                        nn.Conv2d(rel_planes * (pow(5, 2) + 1), out_planes // share_planes, kernel_size=1, bias=False),
                                        nn.BatchNorm2d(out_planes // share_planes), 
                                        nn.ReLU(inplace=True),
                                        nn.Conv2d(out_planes // share_planes, pow(5, 2) * out_planes // share_planes, kernel_size=1))
                                        
            self.unfold_i = nn.Unfold(kernel_size=1, dilation=dilation, padding=0, stride=stride)
            self.unfold_j = nn.Unfold(5, dilation=1, padding=1, stride=stride)
            self.pad = nn.ReflectionPad2d(1)
        self.aggregation = Aggregation(5, stride, (dilation * (kernel_size- 1) + 1) // 2, dilation, pad_mode=1)
    ...
    def forward(self, x):
        ...
        if self.sa_type == 0:  # pairwise
            ...
            w = w_convW.view(x.shape[0], -1, pow(5, 2), x1.shape[-1]) 
